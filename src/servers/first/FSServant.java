...
public class FSServant implements IFS {

    public MyHashMap<Character,MyArrayList<Client>> passengers;
    public MyHashMap<Integer,Flight> flights;
    public static String cityOrigin;
    private static Logger generalLogger = Logger.getLogger("GENERAL LOG");
    private static Logger actionLogger = Logger.getLogger("ACTION LOG");
    private int countPort;

    public void init(int countPort) {
      flights = new MyHashMap<>();
      passengers = new MyHashMap<>();
      this.countPort = countPort;
    }

    public static void initLoggingSystem(String city) {
      cityOrigin = city;

      actionLogger.setLevel(Level.ALL);
      FileHandler aFileHandler;
      try {
        aFileHandler = new FileHandler(cityOrigin.toLowerCase()
                + "/actions.log", true);
      ...
      try {
        uFileHandler = new FileHandler(cityOrigin.toLowerCase()
                + "/general.log", true);
    ...
    public String getBookedFlightCount() {
      ...
      try {
        ...
        for (int i = 0; i < Server.NUMBER-1; i++) {
    ...
    public int transferReservation(String clientId, String currentCity, String otherCity) {
      ...
      synchronized (flight) {
        ...
        try {
          ...
          DatagramPacket request = new DatagramPacket(data.toString().getBytes(), data.length(), ip, Server.transferPorts.get(otherCity));
